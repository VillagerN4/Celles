...
function moveMap() {
    ...
    if(selUnit == null){
        ...
        if(!((selUnit.faction == "nazis" && gameState.activePlayer == "nazis") || (gameState.activePlayer != "nazis" && selUnit.faction != "nazis"))){
            $(".player_dep").hide();
        }else{
            $(".player_dep").show();
        }

        unit_preview.src = `assets/ui/unit_preview/${selUnit.model}.png`;
        ...
        unit_detail_attack.innerHTML = `ATTACK|MOVEMENT|DEFENSE:<br>[${selUnit.attack}|${selUnit.defense}|${selUnit.movement}]`;
        ...
        unit_detail_supply.innerHTML = `${selUnit.supplyState.toUpperCase()}`;

        $("#tab_unit_notavailable").hide();
        $("#tab_unit_available").show();

        
        if(selUnit.disrupted){
            $("#unit_detail_disrupted").show();
        }else{
            $("#unit_detail_disrupted").hide();
        }
        if(selUnit.used){
            $("#unit_detail_used").show();
        }else{
            $("#unit_detail_used").hide();
        }
    }
